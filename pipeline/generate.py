...
# python3 -m pipeline.generate --temperature 1.5 --seed 10 --top_p 0.9 --model mistral-7b-hf --model_type instruct --max_length_of_generated_sequence 50 --dataset coqa # temperature and top_p decided from validation, max_length was restricted for supressing Mistral from hallucinating stories
...
############ for exact same TRIVIAQA dataset generations as well as the train/test split from Conformal Language Modeling paper ###############################
    ...
    if args.prompt_type == 'direct': # orig_settings: for generating answers to the dataset questions
        for batch_idx, batch in tqdm.tqdm(enumerate(dataloader), total=len(dataloader)):
            ...
            if args.decoding_method == 'beam_search':
                ...
                if args.dataset != 'triviaqa': # adding if statement to these orig settings
