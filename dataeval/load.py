...
@functools.lru_cache(maxsize=4)
def read_gpt_eval(path:str, clean=True, debug=False, parallel=False, ith=0, read_only=False, gpt_eval=False):
    ...
    if evals is None:
        ...
        evals = lw._get_gpt_eval(cleaned_sequences, clean, ith, dataset=dataset, logger=logger, parallel=parallel) # evals = [(top_output, [top 5 outputs], [prob of top 5 outputs] for the first cleaned_sequence), (top_output, [top 5 outputs], [prob of top 5 outputs] for the second cleaned_sequence),....]
        evals = {_['id']: _eval for _, _eval in zip(cleaned_sequences, evals)} # prefixing id to evals: {'id_0': (top_output, [top 5 outputs], [prob of top 5 outputs]), 'id_1':....}
        ptd.manual_cache(key, evals, write=not debug)

    ##### saving file with top 5 scores and their prob only while running load.py (not while running pipeline.uq_bb, so default value of gpt_eval is False) ########
    if gpt_eval:
        file_name = path[0:-5]
        file_name += 'gpt_eval/'
...
def read_rouges_new(path:str, clean=True, debug=False, parallel=False):
    ...
    key = get_key_from_generated_strings_path_new(path)
    if clean: key += '_cleaned'
    rouges = ptd.manual_cache(key) # this does not work with the notebook main.ipynb (some path issues in the notebook)
    if rouges is None:
        cleaned_sequences = read_cleaned_outputs_new(path)[:5 if debug else None]
...
@functools.lru_cache(maxsize=4)
def read_deberta_scores(path:str, clean=True, debug=False, parallel=False):
    # alternative to GPT and rougeL evaluation, using Deberta
    key = get_key_from_generated_strings_path_new(path)
    if clean: key += '_cleaned'
    deberta_scores = ptd.manual_cache(key) # this does not work with the notebook main.ipynb (some path issues in the notebook)
    if deberta_scores is None:
        cleaned_sequences = read_cleaned_outputs_new(path)[:5 if debug else None]
        name = f'read_deberta_scores#{key}'
        logger = utils.get_logger(name, os.path.join(os.path.dirname(path), f'{name}.log'), propagate=False)
        if parallel:
            deberta_scores = lw._get_deberta_scores_parallel(cleaned_sequences, clean, logger)
        else:
            deberta_scores = lw._get_deberta_scores(cleaned_sequences, clean, logger)
        ptd.manual_cache(key, deberta_scores, write=not debug)
...
def read_self_eval(path:str, device=None, clean=True, debug=False):
    ...
    if results is None:
        ...
        logger = utils.get_logger(name, os.path.join(os.path.dirname(path), f'{name}.log'), propagate=False)
        model, tokenizer = models.load_model_and_tokenizer(_get_model_name(path), device)
        dataset = 'triviaqa' if 'triviaqa' in path else ('nq_open' if 'nq_open' in path else 'coqa' if 'coqa' in path else 'med_quad')
...
if __name__ == '__main__':
    ...
    for data, _ in _settings.GEN_PATHS.items():
        for model, path in _.items():
            ...
            if model != 'gpt-4-turbo-preview': # if the base model (whose uncertainty is being calculated) is not GPT
