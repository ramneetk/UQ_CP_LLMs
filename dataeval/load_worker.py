...
# in addition to rouge results, they are also returning exact_match results
...
def _compare_generated_texts_to_answers(preds, reference_answers, deberta=False):
    pred_map = {pred: pred.lstrip().lower() for pred in preds}
    if deberta:
        results = {_: _calc_deberta_entailment_scores(_, reference_answers) for _ in pred_map.values()}
    else: # rouge score
...
def _get_rouge_sample(row, text_key=None):
    ...
    all_results = _compare_generated_texts_to_answers(all_preds, all_ans, deberta=False)
    curr = {'id': row['id']}
...
def _get_deberta_scores_sample(row, text_key=None):
    _get_text = lambda x: x if text_key is None else x[text_key]
    all_ans = [row['answer']]
    if 'additional_answers' in row and row['additional_answers'] is not None:
        all_ans += row['additional_answers']
    all_preds = _get_text(row['generations'])
    all_results = _compare_generated_texts_to_answers(all_preds, all_ans, deberta=True)
    curr = {'id': row['id']}
    curr['generations'] = [all_results[_] for _ in _get_text(row['generations'])]
    return curr
...
def _get_deberta_scores(samples, clean:bool, logger=None):
    ...
    ret = []
    for sample in tqdm.tqdm(samples):
        ret.append(_get_deberta_scores_sample(sample, text_key))
    return ret
